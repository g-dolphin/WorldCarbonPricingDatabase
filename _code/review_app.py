...
def load_discovery_review() -> pd.DataFrame:
    if not DISCOVERY_REVIEW_PATH.exists():
        return pd.DataFrame(
            columns=["candidate_id", "decision", "reviewed_at", "reviewer", "comment"]
        )
    return pd.read_csv(DISCOVERY_REVIEW_PATH, dtype=str)


def save_discovery_review(df: pd.DataFrame) -> None:
    df.to_csv(DISCOVERY_REVIEW_PATH, index=False)
...
def _run_discovery_action() -> None:
    with st.spinner("Running discovery..."):
        result = subprocess.run(
            ["python3", "-m", "_code._sources_extraction.cli", "discover"],
            capture_output=True,
            text=True,
            check=False,
        )
...
def _resolve_candidate_path(local_path: str) -> Optional[Path]:
    if not local_path:
        return None
    if str(local_path).strip().lower() in {"nan", "none"}:
        return None
...
def load_rebates_data(gas_label: str) -> dict[str, Any]:
    ...
    tax_relief_records = getattr(module, "tax_relief_records", None)
    tax_exemptions_records = getattr(module, "tax_exemptions_records", None)
    tax_exemptions = getattr(module, "tax_exemptions", [])
    tax_exemptions_sources = getattr(module, "tax_exemptions_sources", [])
    return {
        "records": tax_relief_records or tax_exemptions_records or [],
...
def _render_rebates_module(records: list[Any]) -> str:
    records_str = pprint.pformat(records, width=120, sort_dicts=True)
    return "\n".join(
        [
            "# Auto-generated by review_app.py",
            f"# Generated at {dt.datetime.utcnow().isoformat(timespec='seconds')}Z",
            "",
            f"tax_relief_records = {records_str}",
            "",
        ]
    )
...
def review_view(reviewer: str) -> None:
    ...
    if df.empty:
        st.warning("No candidates found at _raw/_sources/cp_candidates.csv")
        return
...
def source_manager_view() -> None:
    ...
    if disc.empty:
        st.caption(
            "No discovery candidates found. Run `python3 -m _code._sources_extraction.cli discover` "
            "to populate _raw/_sources/discovery_candidates.csv."
        )
    else:
        review_df = load_discovery_review()
        known_urls = set(
            u.strip().lower()
        ...
        st.markdown("**Discovery review decision**")
        if DISCOVERY_REVIEW_PATH.exists():
            st.caption(f"Review file: {DISCOVERY_REVIEW_PATH}")
        else:
            st.caption("No discovery review file yet; it will be created on save.")
        ...
        if "candidate_id" in dq.columns and not dq.empty:
            ...
            existing_review = (
                review_df[review_df["candidate_id"] == selected_disc_id].iloc[0]
                if not review_df.empty and (review_df["candidate_id"] == selected_disc_id).any()
                else None
            )

            decision_default = (
                str(existing_review.get("decision", "")).strip()
                if existing_review is not None
                else ""
            )
            decision_default = decision_default if decision_default in {"accepted", "rejected", "needs_review"} else ""
            decision = st.selectbox(
                "Decision",
                options=["", "accepted", "rejected", "needs_review"],
                index=["", "accepted", "rejected", "needs_review"].index(decision_default or ""),
                key="discovery_decision_select",
            )
            reviewer = st.text_input(
                "Reviewer",
                value=str(existing_review.get("reviewer", "")).strip() if existing_review is not None else "",
                key="discovery_reviewer_input",
            )
            comment = st.text_area(
                "Comment",
                value=str(existing_review.get("comment", "")).strip() if existing_review is not None else "",
                key="discovery_comment_input",
            )
            if st.button("Save discovery decision"):
                now = dt.datetime.now(dt.timezone.utc).isoformat(timespec="seconds")
                new_row = {
                    "candidate_id": selected_disc_id,
                    "decision": decision,
                    "reviewed_at": now,
                    "reviewer": reviewer,
                    "comment": comment,
                ...
                if review_df.empty:
                    review_df = pd.DataFrame([new_row])
                else:
                    review_df = review_df[review_df["candidate_id"] != selected_disc_id]
                    review_df = pd.concat([review_df, pd.DataFrame([new_row])], ignore_index=True)
                save_discovery_review(review_df)
                st.success("Discovery decision saved.")
                st.rerun()
            existing_ids = {
                _normalize_source_id(v)
                for v in df["source_id"].dropna().astype(str).tolist()
            ...
            if st.button("Promote accepted decisions"):
                with st.spinner("Promoting accepted discovery candidates..."):
                    result = subprocess.run(
                        [
                            "python3",
                            "-m",
                ...
                if result.returncode == 0:
                    st.success("Promotion complete. sources.csv updated.")
                    st.cache_data.clear()
                    st.rerun()
                else:
                    st.error("Promotion failed. See details below.")
                    st.code((result.stdout or "") + "\n" + (result.stderr or ""))
...
def _flatten_rebates(records: list[Any], exemptions: list[Any], sources: list[Any]) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        if year_from is None or year_to is None:
            continue
        value_by_year = record.get("value_by_year", {})
        source_by_year = record.get("source_by_year", {})
        default_value = record.get("value", "")
        default_source = record.get("source", "")
        for year in range(int(year_from), int(year_to) + 1):
            rows.append(
                {
                    "year": year,
                    "jurisdictions": record.get("jurisdiction", []) or [],
                    "ipcc": record.get("ipcc", []) or [],
                    "fuels": record.get("fuel", []) or [],
                    "value": value_by_year.get(year, default_value),
                    "source": source_by_year.get(year, default_source),
                    "scheme_id": record.get("scheme_id", ""),
                    "relief_kind": record.get("relief_kind", "exemption"),
                    "relief_base": record.get("relief_base", "tax_rate"),
                }
            )
            ...
            rows.append(
                {
                    ...
                    "relief_kind": "exemption",
                    "relief_base": "tax_rate",
                }
            )
    return rows
...
def rebates_editor_view(reviewer: str) -> None:
    st.subheader("Tax reliefs")

    gas_label = st.selectbox("Gas (tax reliefs)", list(GAS_OPTIONS.keys()), key="rebates_gas")
    rebates_data = load_rebates_data(gas_label)
    records = list(rebates_data["records"])
    exemptions = list(rebates_data["exemptions"])
    sources = list(rebates_data["sources"])
    rebates_path = rebates_data["path"]

    flattened = _flatten_rebates(records, exemptions, sources)
    if flattened:
        st.caption("Existing tax reliefs (sample)")
    ...
    year = int(
        st.number_input(
            "Year (tax relief)", min_value=1990, max_value=2100, value=2024, key="rebate_year"
        )
    )
    ...
    jurisdictions = st.multiselect("Jurisdictions (tax relief)", jur_options)
    rebate_ipcc_options = load_ipcc_codes(gas_label)
    rebate_ipcc_options = _merge_ipcc_options(rebate_ipcc_options, [])
    ipcc_name_map = load_ipcc_name_map()
    ipcc_codes = st.multiselect(
        "IPCC codes (tax relief)",
        rebate_ipcc_options,
        key="rebate_ipcc",
        format_func=lambda c: _ipcc_display(c, ipcc_name_map),
    )
    fuels = st.multiselect(
        "Fuels (tax relief)", load_price_products(), key="rebate_fuels"
    )
    relief_kind = st.selectbox(
        "Relief kind",
        ["exemption", "allowance", "offset"],
        key="rebate_relief_kind",
    )
    relief_base = st.selectbox(
        "Relief base",
        ["tax_rate", "taxable_emissions"],
        index=0 if relief_kind == "exemption" else 1,
        key="rebate_relief_base",
    )
    value = st.number_input("Relief value (0-1)", min_value=0.0, max_value=1.0, value=0.0)
    sources_df = load_sources()
    source = _render_source_picker(
        "Source (tax relief)",
        scheme_id=scheme_id,
    ...
    for row in flattened:
        ...
        if not (set(row["fuels"]) & set(fuels)):
            continue
        if row.get("relief_kind", "exemption") != relief_kind:
            continue
        if row.get("relief_base", "tax_rate") != relief_base:
            continue
        if _is_nonempty(row["value"]) and str(row["value"]).strip() != str(value).strip():
            conflict = True
            conflict_details.append(
                f"Existing value {row['value']} for overlapping tax relief entry"
            )
            break

    confirm = True
    if conflict:
        st.warning("Conflict with existing non-empty tax relief entry.")
        for detail in conflict_details:
            st.write(f"- {detail}")
        confirm = st.checkbox("Confirm overwrite despite conflict", value=False, key="rebate_confirm")

    if st.button("Write tax relief entry", type="primary"):
        if conflict and not confirm:
            st.error("Resolve conflict confirmation before writing.")
            return
        if not scheme_id:
        ...
        new_record = {
            ...
            "year_from": year,
            "year_to": year,
            "relief_kind": relief_kind,
            "relief_base": relief_base,
            "value": value,
            "source": source,
        }
        ...
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(
            _render_rebates_module(records), encoding="utf-8"
        )
        st.success(f"Wrote new tax relief file: {out_path}")
...
def gap_dashboard_view() -> None:
    ...
    if raw_rows:
        ...
        st.caption(
            "Writes _raw/_sources/discovery_queries.csv with queries including scheme name and year."
        )
